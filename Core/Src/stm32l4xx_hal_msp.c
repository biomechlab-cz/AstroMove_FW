...
void HAL_SD_MspInit(SD_HandleTypeDef* hsd)
{
  ...
  */
    ...
    */
    ...
    SDMMC1->MASK = 0;   /* clear any stale interrupt enables */
    /* SDMMC1_IRQn required for DMA mode: DMA TC → HAL_SD_DMA_TxCplt → enables DATAEND
       in SDMMC MASK → SDMMC1_IRQHandler → HAL_SD_TxCpltCallback → BSP_SD_WriteCpltCallback */
    HAL_NVIC_SetPriority(SDMMC1_IRQn, 5, 0);
    HAL_NVIC_EnableIRQ(SDMMC1_IRQn);
