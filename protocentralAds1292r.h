...
#define OFFSETCAL 0x1A // Offset Calibration
...
{
public:
  // TODO add read raw ECG buff data
  ...
  static void ads1292Init(const int chipSelect, const int pwdnPin, const int startPin, const int dataReadyPin);
  static void ads1292Reset(const int pwdnPin);
  void interruptDataReady();
  void addSem(SemaphoreHandle_t *sem);
...
private:
  ...
  static int32_t interpret24bitAsInt32(uint8_t byte24_16, uint8_t byte16_8, uint8_t byte8_0);

  bool dataReadyFlag;
  uint64_t preLastTime;
  uint64_t lastTime;
