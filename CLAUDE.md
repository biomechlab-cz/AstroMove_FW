...
- Each device staggers its pulse by a UID-derived delay (`SYNC_PULSE_STAGGER_MS`, 0–511 ms; FNV-1a hash of the **full 96-bit UID** via `uid_stagger_ms()`, not `w0 % 512` — so same-wafer boards that share the low UID bits get distinct delays and don't tie for group-id leader) so devices booting together off one power source don't pulse simultaneously (coincident pulses merge on the wired-AND line → no distinct peer edge observed). The last (separated) pulse is the common edge everyone latches.
- **Group id (pairing):** layered on the same line, distinguished by LOW duration — short (~20 ms) = sample pulse (reference, above); long (~60 ms start) = group-id frame. Once the bus is quiet, the **leader** (shortest stagger) broadcasts a 16-bit id (`ACQ_SyncSeed()`, from ADS entropy) as `[~60 ms start][16 id][8 checksum]` ×3 at ~8 ms/bit; everyone decodes it. Collision (two leaders) → readback mismatch → id collapses to 0. Stored as the **`group_id` control-CSV column** (NOT the binary header — keeps it plaintext-groupable; the EMX/decoder/golden are unchanged). The frame never moves the sample reference. Inspector pairs cards by equal non-zero `group_id`. Most reliable with simultaneous power-on (leader election); sample alignment is unaffected by power-on timing.
