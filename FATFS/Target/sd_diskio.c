...
/* FatFS validate() calls disk_status()->SD_status()->SD_CheckStatus() on every
 * f_read/f_write.  SD_CheckStatus sends CMD13; if the card is momentarily not
 * in TRANSFER state it returns STA_NOINIT → FR_INVALID_OBJECT.
 * Fix: redirect the SD_Driver vtable to SD_status_cached (defined in
 * beforeFunctionSection below) which returns the cached Stat variable instead.
 * The generated SD_status body becomes dead code but still compiles cleanly. */
#define SD_status SD_status_cached
...
/* Un-rename SD_status so the generated function below compiles (it is dead code —
 * the vtable uses SD_status_cached, not the generated SD_status). */
#undef SD_status
/* Cached SD_status: no CMD13, returns Stat set by SD_initialize. */
DSTATUS SD_status_cached(BYTE lun) { (void)lun; return Stat; }
...
/* Use polling-mode reads for FatFS layer.
 * BSP_SD_ReadBlocks_DMA path requires READ_CPLT_MSG via DMA+SDMMC callback chain.
 * Polling (HAL_SD_ReadBlocks) is 100% CPU-driven — no callbacks, no DMA state
 * conflicts, no race between DMA TC and SDMMC DATAEND.
 * READ_CPLT_MSG is posted manually so SD_read's osMessageQueueGet returns
 * immediately without blocking for 30 s. */
extern SD_HandleTypeDef hsd1;
static uint8_t _bsp_sd_read_poll(uint32_t *p, uint32_t addr, uint32_t n)
{
    if (HAL_SD_ReadBlocks(&hsd1, (uint8_t *)p, addr, n, 5000) != HAL_OK) return MSD_ERROR;
    const uint16_t msg = READ_CPLT_MSG;
    osMessageQueuePut(SDQueueID, (const void *)&msg, 0, 0);
    return MSD_OK;
}
#define BSP_SD_ReadBlocks_DMA _bsp_sd_read_poll
